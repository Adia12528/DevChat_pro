      ...
      {liveKitToken && (
        <div className="livestream-fullscreen-container" style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', marginTop: 60, minHeight: 0 }}>
            ...
            {/* State for comment box position, minimization */}
            {(() => {
              const [commentBoxMinimized, setCommentBoxMinimized] = React.useState(false);
              const [commentBoxPos, setCommentBoxPos] = React.useState({ x: 20, y: isMobileView ? window.innerHeight - 180 : window.innerHeight - 260 });
              ...
              React.useEffect(() => {
                ...
                const handleResize = () => {
                  setCommentBoxPos(pos => ({ x: pos.x, y: isMobileView ? window.innerHeight - 180 : window.innerHeight - 260 }));
                };
                window.addEventListener('resize', handleResize);
                return () => window.removeEventListener('resize', handleResize);
              }, [isMobileView]);
              ...
              const onDrag = (e) => {
                ...
                y = Math.max(8, Math.min(y, window.innerHeight - (commentBoxMinimized ? 48 : (isMobileView ? 140 : 220))));
              ...
              return (
                <div
                  ref={commentBoxRef}
                  className="livestream-comments-panel-floating"
                  style={{
                    position: 'fixed',
                    left: commentBoxPos.x,
                    top: commentBoxPos.y,
                    width: isMobileView ? 260 : 340,
                    minHeight: commentBoxMinimized ? 48 : (isMobileView ? 140 : 220),
                    maxHeight: commentBoxMinimized ? 48 : (isMobileView ? 220 : 320),
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() => setCommentBoxMinimized(min => !min)}
                      style={{ background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: '#00a884', marginLeft: 8 }}
                  ...
                  {!commentBoxMinimized && (
                    <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px', maxHeight: isMobileView ? 80 : 140 }}>
                  ...
                  {!commentBoxMinimized && !isStreamHost && (
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 12px', borderTop: '1px solid #eee', background: '#fafafa' }}>
                      <input
                        type="text"
                        ...
                        style={{ flex: 1, padding: '7px 10px', borderRadius: 6, border: '1px solid #ddd', fontSize: 14 }}
                        maxLength={200}
                      ...
                      <button
                        ...
                        style={{ background: '#00a884', color: '#fff', border: 'none', borderRadius: 6, padding: '7px 12px', fontWeight: 600, fontSize: 14, cursor: livestreamCommentInput.trim() ? 'pointer' : 'not-allowed' }}
                      >
                        Send
                      </button>
