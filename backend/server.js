const express = require('express');
const http = require('http');
const { Server } = require('socket.io');
const mongoose = require('mongoose');
const cors = require('cors');
require('dotenv').config();

const app = express();

// 🛠️ HACK: Keep-Alive Route
app.get('/ping', (req, res) => res.status(200).send('pong'));

app.use(cors());
app.use(express.json());

const server = http.createServer(app);

// 🔌 Optimized Socket.io Config
const io = new Server(server, { 
    cors: { 
        origin: "*", // For development; replace with your Vercel URL in production
        methods: ["GET", "POST"]
    },
    transports: ['websocket'], // 🚀 FORCE WEBSOCKET (Stops reconnect loop)
    pingTimeout: 60000,
    pingInterval: 25000
});

const PORT = process.env.PORT || 5000;
const dbURI = process.env.MONGO_URI || 'mongodb+srv://adia12528_db_user:Adi12528%40as@cluster0.da3qkei.mongodb.net/devchat?retryWrites=true&w=majority';

mongoose.connect(dbURI)
    .then(() => console.log("💎 DB Connected"))
    .catch(err => console.error("❌ DB Error:", err));

const MsgSchema = new mongoose.Schema({
    room: { type: String, index: true },
    sender: String,
    text: String,
    type: { type: String, default: 'text' },
    time: { type: Date, default: Date.now }
});
const Message = mongoose.model('Message', MsgSchema);

io.on('connection', (socket) => {
    console.log(`✅ New connection: ${socket.id}`);

    socket.on('join_room', async (room) => {
        socket.join(room);
        // Fetch last 50 messages
        const history = await Message.find({ room }).sort({ time: 1 }).limit(50);
        socket.emit('load_history', history);
    });

    socket.on('send_message', async (data) => {
        if (!data.text.trim()) return;
        const isImage = /\.(jpg|jpeg|png|webp|gif)$/.test(data.text.toLowerCase());
        const newMessage = new Message({ ...data, type: isImage ? 'image' : 'text' });
        await newMessage.save();
        io.to(data.room).emit("receive_message", newMessage);
    });

    socket.on('clear_chat', async (room) => {
        await Message.deleteMany({ room });
        io.to(room).emit('chat_cleared');
    });

    socket.on('error', (err) => console.log("Socket Error:", err));
});

server.listen(PORT, () => console.log(`🚀 Core Live on ${PORT}`));